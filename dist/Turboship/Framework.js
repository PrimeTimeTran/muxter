...
var Framework = /*#__PURE__*/function () {
  function Framework(name, options, entities, zip) {
  ...
  _createClass(Framework, [{
    ...
    value: function createDirectories() {
      var _this = this;
      this.framework.rootDirectories.forEach(function (dir) {
        _this.zip.folder("".concat(_this.name, "/").concat(dir));
      });
      return this.zip;
    }
  }, {
    key: "zipBaseDirectory",
    value: function zipBaseDirectory() {
      var basePath = "".concat(__dirname, "/nuxt");
        ...
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            case 0:
              ...
              _context.next = 3;
              return new Generator(entities, this.options, this.zip);
            case 3:
              generator = _context.sent;
              _context.next = 6;
              return generator.buildGenesis();
            case 6:
    ...
    value: function setup() {
      var framework = frameworkMap[this.name];
      var root = '/tmp/turboship/' + framework.name;
      try {
        fs.mkdirSync(root, {
          recursive: true
        });
        framework.rootDirectories.forEach(function (dir) {
          var fullPath = "".concat(root, "/").concat(dir);
          try {
...
function getZippedFolderSync(dir, zip) {
  var allPaths = getFilePathsRecursiveSync(dir);
  zip.sync(function () {
    var _iterator = _createForOfIteratorHelper(allPaths),
      _step;
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var filePath = _step.value;
        var addPath = path.relative(path.join(dir, '..'), filePath);
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      if (stat && stat.isDirectory()) {
        var res = getFilePathsRecursiveSync(file);
...
export var frameworkMap = {
  nuxt: {
    ...
    apiContent: {
      'index.get.': function indexGet(label) {
        return "export default defineEventHandler(async (e) => {\n          let { limit, page } = e.context\n          let params = getQuery(e)\n          const query = buildQuery(params)\n          const pipeline = buildPipeline(query, page, limit)\n          const results = await ".concat(label, ".aggregate(pipeline)\n\n          const { data, totalCount } = results[0]\n\n          const response = {\n            meta: {\n              page,\n              pageCount: Math.ceil(parseInt(totalCount[0].total) / limit),\n              totalCount: totalCount.length > 0 ? totalCount[0].total : 0,\n            },\n            data,\n          }\n          return response\n        })\n      ");
      },
      'index.post.': function indexPost(label) {
        return "export default defineEventHandler(async (event) => {\n          const body = await readBody(event)\n          try {\n            return await new ".concat(label, "(body).save()\n          } catch (error) {\n            return error\n          }\n        })\n      ");
      },
      '[_id].delete.': function _idDelete(label) {
        return " export default defineEventHandler(async (event) => {\n          try {\n            return await ".concat(label, ".findOneAndDelete({ _id: event.context.params?._id })\n          }\n          catch (error) {\n            return error\n          }\n        })\n      ");
      },
      '[_id].get.': function _idGet(label) {
        return "export default defineEventHandler(async (event) => {\n          try {\n            return await ".concat(label, ".findOne({ _id: event.context.params?._id })\n          } catch (error) {\n            return error\n          }\n        })\n      ");
      },
      '[_id].put.': function _idPut(label) {
        return "export default defineEventHandler(async (event) => {\n          const body = await readBody(event)\n          try {\n            return await ".concat(label, ".findOneAndUpdate(\n              { _id: event.context.params?._id },\n              body,\n              { new: true }\n            )\n          } catch (error) {\n            return error\n              }\n        })\n      ");
      }
    }
  },
  flutter: {
