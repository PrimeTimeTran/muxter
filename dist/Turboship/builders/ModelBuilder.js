...
class ModelBuilder {
  ...
  getModelPath() {
    ...
    return _path.default.join(currentWorkingDir, 'src', 'Models');
  }
  buildEntities = () => {
    this.entities.map(e => {
      this.e = e;
      ...
      attributes.forEach(f => {
        if (f.name !== '_id') {
          ...
          if (f.type === 'enumerator' || f.type === 'enumeratorMulti') {
            ...
            const options = (_f$options = f.options) == null ? void 0 : _f$options.split(',');
            if (options) {
              options.forEach(o => {
  ...
  generateFields = (fields, name) => {
    ...
    for (const key of keys) {
      ...
      values.push(`${(0, _helpers.camelize)(key)}: ${(0, _helpers.getType)(name, type, key)}${!required ? '.optional()' : ''}`);
    }
    ...
    for (const f in this.e.fields) {
      ...
      if (type === 'relation') {
        ...
        const item = `${fieldName}: {
          ...
          type: [
            ${(0, _helpers.capitalize)(enumeratorType)}
          ],
        }`;
        values.push(item);
      } else {
        ...
        const item = `${fieldName}: {
          type: ${this.getType((0, _helpers.capitalize)(type))},
          ${required != undefined ? `required: ${required},` : ''}
        }`;
        values.push(item);
      }
